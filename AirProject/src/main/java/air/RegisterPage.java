...
public class RegisterPage extends HttpServlet {
		...
		try {
		        ...
		        if(Integer.parseInt(rs.getString("amount"))==0) {
		        	if(request.getParameter("pwd").equals(request.getParameter("pwd_r"))) {
		        		sql = "INSERT INTO `users` (`id`, `name`, `surname`, `email`, `role`, `pwd`) VALUES (NULL, '"+request.getParameter("name")+"', '"+request.getParameter("surname")+"', '"+request.getParameter("email")+"', 'client', '"+request.getParameter("pwd")+"')";
		        		...
		        		session.setAttribute("name", rs.getString("surname"));
	...
	public void printBody(PrintWriter out) {
		...
		out.println("      <input type=\"text\" class=\"form-control\" name=\"name\" id=\"name\" placeholder=\"Name\"required> ");
		out.println("    </div>");
		out.println("    <div class=\"form-group\">");
		out.println("      <label for=\"surname\">Password</label>");
		out.println("      <input type=\"text\" class=\"form-control\" name=\"surname\" id=\"surname\" placeholder=\"Surname\"required> ");
		out.println("    </div>");
		out.println("    <div class=\"form-group\">");
		...
		out.println("  <p class=\"mt-3\">Already have an account? <a href=\"MainPage\">Login here</a></p>");
		out.println("  </form>");
		out.println("</div>");
	}
