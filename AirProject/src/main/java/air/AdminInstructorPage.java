...
/**
 * Servlet implementation class AdminInstructorPage
 */
@WebServlet("/AdminInstructorPage")
public class AdminInstructorPage extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(operation.equals("add")) {
			addInstructor(out,request,session,response);
			response.sendRedirect(request.getContextPath()+"/AdminInstructorPage");
		}
		if(operation.equals("edit")) {
			updateInstructor(out,request,session,response);
			response.sendRedirect(request.getContextPath()+"/AdminInstructorPage");
		}
		if(operation.equals("delete")) {
			deleteInstructor(out,request,session,response);
			response.sendRedirect(request.getContextPath()+"/AdminInstructorPage");
		}
	...
	private void addInstructor(PrintWriter out, HttpServletRequest request, HttpSession session, HttpServletResponse response) {
		 String sql = "INSERT INTO `users` (`id`, `name`, `surname`, `email`, `pwd`, `role`) VALUES (NULL, '"+request.getParameter("name")+"', '"+request.getParameter("surname")+"', '"+request.getParameter("name").toLowerCase()+"."+request.getParameter("surname").toLowerCase()+"@gmail.com', '123', 'instructor')";
		 try {
	...
	private void deleteInstructor(PrintWriter out, HttpServletRequest request, HttpSession session, HttpServletResponse response) {
		String check = "SELECT COUNT(*) as amount FROM flights WHERE pilot_id =  "+request.getParameter("id")+";";
		try {
		     stmt = con.createStatement();
	...
	private void updateInstructor(PrintWriter out, HttpServletRequest request, HttpSession session, HttpServletResponse response) {
		 String sql = "UPDATE users SET name = '"+request.getParameter("name")+"', surname = '"+request.getParameter("surname")+"',active = '"+request.getParameter("active")+"' WHERE id = '"+request.getParameter("id")+"';";
		 try {
		     stmt = con.createStatement();
		     stmt.executeUpdate(sql);
			 } catch (SQLException e) {
	...
	public void printBody(PrintWriter out,HttpSession session) {
		...
		out.println("<h2>Manage instructors</h2>");
		  ...
		  String sql = "SELECT * FROM users WHERE role='instructor';";
			try {
				...
				while(rs.next()) {
					  out.println("    <tr>");
					  out.println("      <form method='POST' action='AdminInstructorPage'>");
					  ...
					  out.println("        <td><input type=\"text\" name=\"surname\" value="+rs.getString("surname")+"></td>");
					  out.println("        <td>"+rs.getString("email")+"</td>");
					  out.println("        <td><input type=\"number\" name=\"active\" value="+rs.getString("active")+"></td>");
		  ...
		  out.println("<h3>Add Instructor</h3>");
		  out.println("<form class=\"form-inline\" method='POST' action='AdminInstructorPage'>");
		  ...
		  out.println("    <label for=\"inputPassword2\" class=\"sr-only\">Surname</label>");
		  out.println("    <input type=\"text\" name='surname' class=\"form-control\" id=\"inputPassword2\" placeholder=\"Surname\">");
		  out.println("  </div>");
		  out.println("  <button type=\"submit\" class=\"btn btn-primary mb-2\">Add Instructor</button>");
		  out.println("</form>");
	}
	public void printHead(PrintWriter out) {
	    ...
	    out.println("        </li>");
	    out.println("<form action=\"AdminInstructorPage\" method=\"post\">");
